...
df_alpha_beta = pd.read_csv("./data_clean_small.csv", index_col=0, sep='\t')

# 统计peptide字段中每个序列的出现次数
peptide_counts = df_alpha_beta['antigen.epitope'].value_counts()

# 找到出现次数最多的序列和对应的出现次数
most_common_peptide = peptide_counts.idxmax()
count_most_common = peptide_counts.max()

df = df_alpha_beta[df_alpha_beta['antigen.epitope'] == 'ATDALMTGF']
# encode label
...
# 初始化模型、优化器和损失函数
model = tcr_bert_TwoPartBertClassifier.TwoPartBertClassifier(pretrained='wukevin/tcr-bert-mlm-only', n_output=2, freeze_encoder=False, separate_encoders=True)
