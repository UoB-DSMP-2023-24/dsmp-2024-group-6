...
def train_model(model, train_loader, val_loader, optimizer, loss_fn, device, epochs=10, early_stopping_patience=3):
    ...
    for epoch in range(epochs):
        ...
        all_probabilities = []
        with torch.no_grad():
            for data in tqdm.tqdm(train_loader, desc="Evaluation: "):
                input_ids_a = data['input_ids_a'].to(device)
                attention_mask_a = data['attention_mask_a'].to(device)
                input_ids_b = data['input_ids_b'].to(device)
                attention_mask_b = data['attention_mask_b'].to(device)
...
# 初始化模型、优化器和损失函数
model = tcr_bert_TwoPartBertClassifier.TwoPartBertClassifier(pretrained='wukevin/tcr-bert-mlm-only', n_output=30, freeze_encoder=False, separate_encoders=True)
...
train_df, remaining_data = train_test_split(df_alpha_beta, test_size=0.4, stratify=df_alpha_beta['antigen.epitope'])
validate_df, test_df = train_test_split(remaining_data, test_size=0.5, stratify=remaining_data['antigen.epitope'])
