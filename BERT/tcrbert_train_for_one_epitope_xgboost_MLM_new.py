...
output_path = "./log/training_log_new.txt"
# data_path = "./data_clean_large_neg.csv"
data_path_prefix = "/Users/berlin/Documents/UoB/DSMP/pycharm_project_DSMP/dsmp-2024-group-6/data/"
...
unique_epitopes = list(reversed(unique_epitopes))
...
for epitope in unique_epitopes:
    ...
    df_train['cdr3_a_aa'] = df_train['cdr3_a_aa'].apply(add_space)
    df_train['cdr3_b_aa'] = df_train['cdr3_b_aa'].apply(add_space)
    df_test['cdr3_a_aa'] = df_test['cdr3_a_aa'].apply(add_space)
    df_test['cdr3_b_aa'] = df_test['cdr3_b_aa'].apply(add_space)
    ...
    df_train_a = df_train[df_train['binder'] == 1]
    df_train_b = df_train[df_train['binder'] == 1]

    # 实例化数据集和数据加载器
    train_dataset = TCRDataset(df_train, tokenizer, 64)
    train_dataset_a = TCRDataset_a(df_train_a, tokenizer, 64)
    train_dataset_b = TCRDataset_b(df_train_b, tokenizer, 64)
    # val_dataset = TCRDataset(validate_df, tokenizer, 64)
    test_dataset = TCRDataset(df_test, tokenizer, 64)
