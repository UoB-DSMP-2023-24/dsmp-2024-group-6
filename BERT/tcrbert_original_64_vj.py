...
df_alpha_beta = pd.read_csv("/home/rnd/wmj/berlin/ttc/data_clean_large.csv", sep='\t')
# encode label
...
model = tcr_bert_TwoPartBertClassifier.TwoPartBertClassifier(pretrained='/home/rnd/wmj/berlin/ttc/tcr-bert-mlm-only', n_output=30, freeze_encoder=False, separate_encoders=True).to(device)
...
for a_row, v_b_gene, j_b_gene, v_a_gene, j_a_gene in zip(output_list, train_df['v_b_gene'], train_df['j_b_gene'], train_df['v_a_gene'], train_df['j_a_gene']):
    # 合并当前行的数据
    merged_row = a_row  + [v_b_gene] + [j_b_gene] + [v_a_gene] + [j_a_gene]
    # 将合并后的行添加到结果列表中
...
# 训练XGBoost模型
clf = xgb.XGBClassifier(objective='multi:softmax', eval_metric='logloss', num_class=30, verbosity=2, tree_method = "hist", device = "cuda")
...
merged_list = []
# 遍历 output_a 和 output_b 中的每一行
for a_row, v_b_gene, j_b_gene, v_a_gene, j_a_gene in zip(output_list, test_df['v_b_gene'], test_df['j_b_gene'], test_df['v_a_gene'], test_df['j_a_gene']):
    # 合并当前行的数据
    merged_row = a_row  + [v_b_gene] + [j_b_gene] + [v_a_gene] + [j_a_gene]
    # 将合并后的行添加到结果列表中
    merged_list.append(merged_row)
...
# 对于多分类问题的AUROC
try:
    auroc = roc_auc_score(label_list, y_score, multi_class='ovr', average='macro')
...
# 对于多分类问题的AUPRC
try:
    auprc = average_precision_score(label_list, y_score, average='macro')
except ValueError as e:
    print(f"无法计算AUPRC: {e}")
    auprc = None
