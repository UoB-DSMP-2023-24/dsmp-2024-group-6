...
# BLOSUM62是一种常用于蛋白质序列比对的替代矩阵，它包含了蛋白质中20个氨基酸的替代得分。
blosum62 = substitution_matrices.load('BLOSUM62')
AAstring = 'ACDEFGHIKLMNPQRSTVWY'
AAstringList = list(AAstring)
cur_dir = os.path.dirname(os.path.realpath(__file__)) + '/'

# 对于矩阵中的每个得分，如果得分大于4，则将其调整为4；否则，保留原来的得分。
# 这样的调整可能是为了处理特定的序列比对任务，通过限制最高得分来避免得分过高的替代对比对结果产生不利影响。
blosum62n = {}
...
# 它通过bl62字典定义了BLOSUM62得分矩阵，这是一种常用于比较蛋白质序列中氨基酸对替代可能性的得分矩阵。
...
# 然后，代码通过对这些得分进行调整（所有得分减去4）和多维尺度分析（MDS）来转换这些得分到一个新的表示空间，
# 旨在保留原始得分矩阵的距离关系。
...
# 通过MDS，它生成了一个低维（在这个例子中是16维）的嵌入空间，其中每个氨基酸的新表示由16个维度的向量构成。
# 这可以用于进一步的生物信息学分析，比如序列对比或聚类，以发现蛋白质或氨基酸序列之间的相似性或功能关联。
...
# 这段代码创建了一个新字典bl62np，用于存储每个氨基酸的新的低维表示。
# 这里，X[ii,]是第ii个氨基酸在低维空间中的坐标，
# 然后通过添加额外的零（[0] * Ndim * 5）来扩展这些坐标向量，以确保每个向量的长度一致。
bl62np = {}
vkk = list(bl62.keys())
for ii in range(20):
    kk = vkk[ii]
...
# 对于序列s中的每个氨基酸，它查找该氨基酸在之前通过MDS处理得到的低维表示（存储在bl62np中），然后使用矩阵M对当前向量x和氨基酸的表示进行线性变换。
# 这个过程迭代进行，直到处理完序列中的所有氨基酸，最终返回序列s的编码向量x。
...
# 它检查序列中的氨基酸是否全部有效，以及序列长度是否在预定义的范围内。
# 如果序列满足条件，它将根据序列长度分类并存储相关信息。如果序列包含非氨基酸字符或长度不在范围内，则会跳过该序列。
...
# CollapseUnique 函数的目的是从数据集中识别并分组唯一的CDR3序列。
def CollapseUnique(LD, VD, ID, SD):
    ...
    IDu = {}
    SDu = {}
    # 即便没有提供变量基因信息，代码仍能正常运行，通过使用默认值填补信息的缺失。
    for kk in kks:
        vvL = list(LD[kk])
        if len(VD) > 0:
            vvV = list(VD[kk])
        else:
            vvV = ['TRAV2-1*01'] * len(vvL)
        vvI = list(ID[kk])
        vvS = list(SD[kk])
...
# 这段代码旨在通过分析CDR3序列的k-mer组成，来探索和识别序列之间的相似性和差异性
class KmerSet:
...
# 这段代码的目的是生成一个序列共享图（SeqShareGraph），其中节点表示序列ID，边表示序列之间共享相同长度和至少一个共同的motif。
...
# 这段代码的目的是生成一个共享序列图（SSG），
# 在这个图中，节点代表CDR3序列，边代表序列间通过共享特定数量（k_thr）的k-mer而形成的连接。
...
# 比较两个序列s1和s2，计算基于BLOSUM62得分矩阵（调整后的版本blosum62n）的相似性得分。
# 如果序列中的氨基酸在特定位置相同，得分增加；
# 如果不同，根据BLOSUM62得分矩阵减去相应的得分。
# 如果存在间隙或特殊字符（'.'，'-'，'*'），则根据情况可能减去固定的间隙惩罚分。
...
# 对两个长度不同的序列进行局部对齐，如果长度差异超过gap_thr，则直接返回-1表示无法对齐。
...
# 在序列中插入一个或两个间隙，生成所有可能的新序列变体，以便进行后续的序列比较。
def InsertGap(Seq, n):
...
# 对两个CDR3序列s1和s2进行比较，基于它们的中间部分（去除前st个和最后2个氨基酸）和可变基因段（V区）的得分来计算相似性得分。
...
#  非递归方式实现的深度优先搜索算法，用于遍历图中的节点，以找到所有从起始节点可达的节点。
def dfs(graph, start):
    '''
...
# 使用深度优先搜索算法对共享序列图（SSG）进行聚类，将相互连接的节点（即共享特定序列特征的序列）分组成簇。
# 函数迭代地选取尚未归入任何簇的节点作为新簇的起始点，直到所有节点都被分配到某个簇中。
...
# 根据V基因得分矩阵进行聚类，类似于IdentifyMotifCluster，但专注于根据V基因的相似性将序列分组。
def IdentifyVgeneCluster(sMat, thr_v=None):
...
# 解析FASTA文件，将每个序列的头信息和对应的序列内容存储到字典中，方便后续的访问和处理。
def ParseFa(fname):
...
# 这段代码的目的是预先计算V基因（可变基因段）之间的距离，
# 首先，它从FASTA文件中解析V基因序列，并将这些序列存储在字典中。
# 然后，对于每对V基因，代码计算CDR1和CDR2区域的序列相似性得分，使用之前定义的SeqComparison函数进行比较。
# 最后，将每对V基因的得分保存到一个文件中。
# 这个预计算过程只需要运行一次，生成的得分可以用于后续的V基因比较任务，加速匹配和聚类分析。
# 这个分数是通过对比这两个区域的序列得到的，分数越高表示序列之间的相似性越高。
# 在这个上下文中，得分80.0很可能是通过累加CDR1和CDR2比较得分得到的最高值，表明了非常高的相似性或完全匹配。
def PreCalculateVgeneDist(VgeneFa="/Users/lifushen/Desktop/giana1/imgt_Human_TRAV.fasta"):
    ...
    gg = open('VgeneScores_alpha.txt', 'w')
    for kk in VScore:
        vv = VScore[kk]
...
def calculate_euclidean_distance_matrix(encoded_sequences):
    # 之前提供的计算距离矩阵的函数实现
    n_sequences = encoded_sequences.shape[0]
    distance_matrix = np.zeros((n_sequences, n_sequences))

    for i in range(n_sequences):
        for j in range(i, n_sequences):
            distance = np.sqrt(np.sum((encoded_sequences[i] - encoded_sequences[j]) ** 2))
            distance_matrix[i, j] = distance
            distance_matrix[j, i] = distance

    return distance_matrix

# 这段代码的目的是对CDR3序列进行编码，并将其分组到不同的聚类中。
# 预处理：从文件中读取CDR3序列和其他信息，如可变基因（如果Vgene=True）。
# 编码和聚类：将CDR3序列编码到96维空间中，并根据编码结果执行k-means聚类
# 如果exact=True，还会在每个聚类内执行Smith-Waterman序列比对，以进一步细分聚类。
# 处理可变基因：如果提供了可变基因信息，将使用它来帮助聚类过程，通过比较可变基因之间的相似度。
# 结果输出：将聚类结果写入输出文件中，包括CDR3序列、聚类ID和其他相关信息。
def EncodeRepertoire(inputfile, outdir, outfile='', exact=True, ST=3, thr_v=3.7, thr_s=3.5, VDict={}, Vgene=True,
                     thr_iso=10, gap=-6, GPU=False, Mat=False, verbose=False):
    ## No V gene version
    ...
    InfoLine = '##TIME:' + tm + '|cmd: ' + sys.argv[0] + '|' + inputfile + '|IsometricDistance_Thr=' + str(
        thr_iso) + '|thr_v=' + str(thr_v) + '|thr_s=' + str(thr_s) + '|exact=' + str(exact) + '|Vgene=' + str(
        Vgene) + '|ST=' + str(ST)
    g.write(InfoLine + '\n')
    ...
    for kk in LDu:
        ...
        Cls = ClusterCDR3(dM, flagL, thr=thr_iso - 0.5 * (15 - kk),
                          verbose=verbose)  ## change cutoff with different lengths


        # 计算距离矩阵
        distance_matrix = calculate_euclidean_distance_matrix(dM)
        print("CDR3序列之间的欧几里得距离矩阵：")
        print(distance_matrix)
        # 假设您的输出文件名为outfile，已经定义好了
        distance_matrix_file = os.path.join(outdir, outfile.replace('.txt', '_distance_matrix.txt'))

        # 使用np.savetxt函数保存距离矩阵到文件
        np.savetxt(distance_matrix_file, distance_matrix, fmt='%.4f')

        print(f"距离矩阵已保存到文件：{distance_matrix_file}")


        if verbose:
            print("     Handling identical CDR3 gr"
                  "oups")
        Cls_u = []
...
# 这段代码的目的是对给定的字典Ig进行排序和筛选，其中Ig可能代表含有特定信息的免疫球蛋白（Ig）序列。
def OrderUnique(Ig):
    vv = list(Ig.values())
    kk = list(Ig.keys())
...
# 它使用Faiss库加速欧氏距离计算，通过迭代的方式将相似的CDR3序列分组到聚类中。
# 在每一轮迭代中，代码通过比较序列之间的距离与设定的阈值thr来决定是否将序列聚合到同一组。
# CDR3序列之间的距离小于等于设定的阈值（在这个例子中是10），它们就会被分到同一个簇里。
    ...
    while 1:
        ...
        if flag == 0:
            ...
            for ii in range(len(vv)):
                ...
                sorted(list(set(list(Ig[kkg[idx1]][1]) + list(
                    Ig[kkg[idx2]][1])))))  ## Second entry records the absolute index of a sequence
...
# 首先，它使用Faiss库创建一个L2距离索引，并将CDR3序列的编码数据dM添加到索引中。
# 然后，执行范围搜索来找出所有与每个序列在指定阈值thr内的邻近序列。
# 通过这些邻近序列信息，算法迭代地将序列分配到聚类中，直到所有序列都被分配。
def ClusterCDR3r(dM, flagL, thr=10, verbose=False):
    ...
    while True:
        ...
        neighbor = np.unique(
            np.array(list(chain(*[I[(lims[cur_idx_i]): lims[cur_idx_i + 1]] for cur_idx_i in cur_idx]))))
...
''')
    ...
    parser.add_option("-S", "--threshold_score", dest="thr_s", default=3.6,
                      help="Threshold for Smith-Waterman alignment score (normalized by CDR3 length). Default 3.6")
    parser.add_option("-G", "--threshold_vgene", dest="thr_v", default=3.7,
    ...
    parser.add_option("-T", "--startPosition", dest='ST', default=3,
    ...
    parser.add_option("-n", "--GapNumber", dest="GapN", default=1,
                      help="Maximum number of gaps allowed when performing alignment. Max=1, default=1. Not used.")
    parser.add_option("-V", "--VariableGeneFa", dest="VFa", default="Imgt_Human_TRBV.fasta",
                      help="IMGT Human beta variable gene sequences")
    parser.add_option("-v", "--VariableGene", dest="V", default=True, action="store_false",
    ...
    parser.add_option("-e", "--Exact", dest="E", default=True, action="store_false",
                      help="If False, GIANA will not perform Smith-Waterman alignment after isometric encoding.")
    parser.add_option("-N", "--NumberOfThreads", dest="NN", default=1,
                      help="Number of threads for multiple processing. Not working so well.")
    parser.add_option("-M", "--EncodingMatrix", dest="Mat", default=False, action="store_true",
                      help="If true, GIANA will export the isometric encoding matrix for each TCR. Default: False.")
    parser.add_option("-U", "--UseGPU", dest="GPU", default=False, action="store_true",
                      help="Use GPU for Faiss indexing. Must be CUDA GPUs.")
    parser.add_option("-q", "--queryFile", dest="Query", default='',
    ...
    parser.add_option("-b", "--Verbose", dest='v', default=False, action="store_true",
                      help="Verbose option: if given, GIANA will print intermediate messages.")
    return parser.parse_args()

# 查询模式：当提供查询文件时，程序会比较这些查询序列与一个参考文件中的序列。
# 首先，它会检查参考文件是否存在，然后创建参考数据结构。
# 接着，对于每个查询文件，程序会生成一个查询聚类文件，并与参考聚类文件合并，最后输出合并后的结果。
#
# 常规聚类模式：当没有提供查询文件时，程序会执行常规的聚类分析。
# 它可以处理单个文件、一个目录下的多个文件或一个文件列表。
# 程序首先预计算V基因间的距离，然后对每个输入文件进行CDR3序列的编码和聚类，最后输出聚类结果。
def main():
    ...
    if len(qFile) > 0:
        ...
        vf = open('./VgeneScores_alpha.txt')  ## Use tcrDist's Vgene 80-score calculation
        VScore = {}
        VV = opt.V
        EE = opt.E
        Mat = opt.Mat
        ...
        for ff in files:
            ...
            EncodeRepertoire(ff, OutDir, OutFile, ST=ST, thr_s=thr_s, thr_v=thr_v, exact=EE, VDict=VScore, Vgene=VV,
                             thr_iso=cutoff, gap=Gap, GPU=GPU, Mat=Mat, verbose=verbose)
