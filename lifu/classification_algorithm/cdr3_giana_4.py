...
# Function to encode columns of non-amino acid sequences using integer encoding
def integer_encode_column(column):
    unique_vals = column.unique()
    int_map = {val: idx for idx, val in enumerate(unique_vals)}
    return column.map(int_map), int_map

# Read input CSV file, assuming space-separated fields
input_file = "/Users/lifushen/Desktop/1d_cnn/data_clean_large_neg.csv"
data = pd.read_csv(input_file, sep='\t', header=0)

# Check if all necessary columns are present
expected_columns = ['cdr3_a_aa','cdr3_b_aa','antigen.epitope','binder']
missing_columns = [col for col in expected_columns if col not in data.columns]
if missing_columns:
    raise ValueError(f"Missing columns: {missing_columns}. Check input file format and column names.")

# Encode amino acid sequences
def extract_middle_six(s):
    if len(s) >= 6:
        middle_index = len(s) // 2
        return s[middle_index - 3: middle_index + 3]
    else:
        return s

data['cdr3_a_aa'] = data['cdr3_a_aa'].apply(lambda s: EncodingCDR3(s, M6, n0))
data['cdr3_b_aa'] = data['cdr3_b_aa'].apply(lambda s: EncodingCDR3(s, M6, n0))

for column in ['antigen.epitope']:
    codes, uniques = pd.factorize(data[column])
    # Increment codes by 2 to avoid 0 and 1
    data[column] = codes + 2  # Now codes start at 2 instead of 0
    # 创建索引字典来映射编码后的整数值到原始字符串
    index_to_epitope = {i + 2: epitope for i, epitope in enumerate(uniques)}

    # 打印出映射的示例，查看如何将编码值映射回原始字符串
    print("编码与原始antigen.epitope的映射示例:")
    for code in sorted(index_to_epitope.keys())[:100]:  # 打印前100个映射作为示例
        print(f"{code} - {index_to_epitope[code]}")


# Reorder columns as specified
new_column_order = ['cdr3_a_aa','cdr3_b_aa','antigen.epitope','binder']

# Remove the first unnamed column and reorder the rest
data = data.loc[:, ~data.columns.str.contains('^Unnamed')]
data = data[new_column_order]

# Save to new CSV file
print("开始保存文件...")
output_file = "/Users/lifushen/Desktop/1d_cnn/output_encoded_4.csv"
