...
input_file = "/Users/lifushen/Desktop/1d_cnn/clean_altogther.txt"
data = pd.read_csv(input_file, sep='\\s+', header=0)

data['encoded_cdr3_b_aa'] = data['cdr3_b_aa'].apply(lambda s: EncodingCDR3(extract_middle_six(s), M6, n0))
data['encoded_cdr3_a_aa'] = data['cdr3_a_aa'].apply(lambda s: EncodingCDR3(extract_middle_six(s), M6, n0))

# 检查必需的列是否存在
expected_columns = ['cdr3_a_aa', 'cdr3_b_aa', 'v_b_gene', 'j_b_gene', 'v_a_gene', 'j_a_gene', 'antigen.epitope']
missing_columns = [col for col in expected_columns if col not in data.columns]
if missing_columns:
    raise ValueError(f"Missing columns: {missing_columns}. Check input file format and column names.")

# 对 antigen.epitope 进行整数编码
encoded_labels, original_labels = pd.factorize(data['antigen.epitope'])

# 将整数编码和原始标签存储在 DataFrame 中
data['encoded_antigen_epitope'] = encoded_labels

# 创建映射字典
mapping_dict = {encoded: original for encoded, original in zip(encoded_labels, data['antigen.epitope'])}

# 打印映射
print("Encoded to Original Label Mapping:")
for encoded, original in mapping_dict.items():
