...
# 读取CSV文件
csv_file_path = '/Users/lifushen/Desktop/giana_distance_matrix_visual/output_encoded_b.csv'  # 请替换为您的CSV文件路径
df = pd.read_csv(csv_file_path)

# 去除字符串中的换行符和逗号，并分割字符串以形成一个数值列表
split_data = df['encoded_CDR3b'].str.replace('\n', ' ').str.replace('[', '').str.replace(']', '').str.replace(',', ' ').str.split()
...
# 保存矩阵到TXT文件
np.savetxt('distance_matrix_b.txt', square_distance_matrix, fmt='%s', delimiter='\t')
