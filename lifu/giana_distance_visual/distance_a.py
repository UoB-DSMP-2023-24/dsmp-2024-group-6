...
# 加载稀疏矩阵和映射
sparse_matrix = pd.read_csv('/Users/lifushen/Desktop/distance_visual/Mat_dist_alpha_M.txt', sep='\t', index_col=0)
cdr3_to_epitope = pd.read_csv('/Users/lifushen/Desktop/distance_visual/map_a_M.txt', sep='\t')
...
epitope_dict = pd.Series(cdr3_to_epitope.epitope.values, index=cdr3_to_epitope.cdr3_a).to_dict()
reduced_df['Epitope'] = reduced_df.index.map(epitope_dict)

# 计算每个epitope出现的频率，并选取出现频率最高的10个
top_epitopes = reduced_df['Epitope'].value_counts().nlargest(5).index

# 创建一个只包含最常见的10个epitope的数据子集
top_epitopes_df = reduced_df[reduced_df['Epitope'].isin(top_epitopes)]

# 为Top 10 epitopes选择一个颜色调色板
palette = sns.color_palette("deep", n_colors=5)  # 修改颜色数量以匹配top_epitopes的数量
...
plt.title('t-SNE Reduced Sparse Matrix Visualization For Alpha - Epitopes')
