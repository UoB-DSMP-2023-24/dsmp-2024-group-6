...
output_txt_path = '/Users/lifushen/Desktop/giana_for_mouse/clean_alpha.txt'  # 输出文件的路径和文件名，扩展名改为.txt

# 使用pandas读取CSV文件，这里假定第一行是字段名，逗号（','）作为分隔符
df = pd.read_csv(input_csv_path, usecols=['cdr3_a_aa', 'v_a_gene', 'j_a_gene', 'antigen.epitope'], sep='\t')
