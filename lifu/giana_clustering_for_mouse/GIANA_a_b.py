...
def EncodeRepertoire(inputfile, outdir, outfile='', exact=True, ST=3, thr_v=3.7, thr_s=3.5, VDict={}, Vgene=True,
    ...
    for kk in LDu:
        ...
        if exact:
            ...
            Cls = Cls_s
        if len(Cls) > 1:
            cluster_labels = np.empty(dM.shape[0], dtype=np.int32)
            for cluster_id, indices in enumerate(Cls):
                cluster_labels[indices] = cluster_id

            if verbose:
                print("labels:", cluster_labels)

            silhouette_avg = silhouette_score(dM, cluster_labels)
            calinski_harabasz = calinski_harabasz_score(dM, cluster_labels)
            davies_bouldin = davies_bouldin_score(dM, cluster_labels)
