...
# df_alpha_beta = df_alpha_beta.iloc[:100]
...
print(train_df.iloc[0]['encoded_epitopes'])

# 训练模型
output_a, output_b, label_list = get_encode_data(model, train_loader, device)

# merged_list = [a + b + c for a, b, c in zip(output_a, output_b, train_df['encoded_epitopes'])]
# merged_list = [a + b for a, b in zip(output_a, output_b)]
merged_list = []
# 遍历 output_a 和 output_b 中的每一行
for a_row, b_row, e in zip(output_a, output_b, train_df['encoded_epitopes']):
    # 合并当前行的数据
    merged_row = a_row + b_row + [e]
    # 将合并后的行添加到结果列表中
...
merged_list = []
# 遍历 output_a 和 output_b 中的每一行
for a_row, b_row, e in zip(output_a, output_b, test_df['encoded_epitopes']):
    # 合并当前行的数据
    merged_row = a_row + b_row + [e]
    # 将合并后的行添加到结果列表中
    merged_list.append(merged_row)
