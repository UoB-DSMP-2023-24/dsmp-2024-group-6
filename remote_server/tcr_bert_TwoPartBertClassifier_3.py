...
class TwoPartBertClassifier(nn.Module):
    ...
    def forward(self, tcr_a, tcr_b, feature_extraction=False):
        # 标准前向传递
        if not feature_extraction:
            # 你之前的forward逻辑
            return self.standard_forward(tcr_a, tcr_b)
        else:
            # 特征提取逻辑
            return self.feature_extraction_forward(tcr_a, tcr_b)

    def feature_extraction_forward(self, tcr_a, tcr_b):
        bs = tcr_a.shape[0]
    ...
    def standard_forward(self, tcr_a, tcr_b):
        bs = tcr_a.shape[0]
        # input of (batch, seq_len) (batch, 20)
        # last_hidden_state (batch, seq_len, hidden_size) (batch, 20, 144)
        # pooler_output (batch_size, hiddensize)
