...
class TCRDataset(Dataset):
    def __init__(self, df, tokenizer, max_length):
        self.cdr3_a_aa = df['cdr3_a_aa'].tolist()
        self.cdr3_b_aa = df['cdr3_b_aa'].tolist()
        self.clusterid = df['clusterid'].tolist()
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'clusterid': torch.tensor(self.clusterid[idx], dtype=torch.long),
...
def get_encode_data(model_a, model_b, train_loader, device, pooling_strategy = "mean"):
    ...
    for data in tqdm.tqdm(train_loader, desc="Getting data Process: "):
        ...
        clusterid = data['clusterid'].to(device)
        ...
        clusterid_list += clusterid.tolist()

    return output_list_a, output_list_b, label_list, clusterid_list
...
output_path = "/home/rnd/wmj/berlin/ttc/log/training_log_clusterid.txt"
data_path = "/home/rnd/wmj/berlin/ttc/data_cluster_large_neg.csv"
...
for epitope in unique_epitopes:
    ...
    train_df_a_b = train_df[train_df['binder'] == 1]

    # 实例化数据集和数据加载器
    train_dataset = TCRDataset(train_df, tokenizer, 64)
    train_dataset_a = TCRDataset_a(train_df_a_b, tokenizer, 64)
    train_dataset_b = TCRDataset_b(train_df_a_b, tokenizer, 64)
    ...
    for epoch in range(0, epochs):
        ...
        # 训练模型
        output_a, output_b, label_list, clusterid_list = get_encode_data(model_a, model_b, train_loader, device, pooling_strategy="cls")

        # merged_list = [a + b for a, b in zip(output_a, output_b)]

        merged_list = []
        # 遍历 output_a 和 output_b 中的每一行
        for a_row, b_row, clusterid in zip(output_a, output_b, clusterid_list):
            # 合并当前行的数据
            merged_row = a_row + b_row + [clusterid]
            # 将合并后的行添加到结果列表中
            merged_list.append(merged_row)

        # # # 将 NumPy 数组转换为 PyTorch 张量
        # # merged_list = torch.tensor(merged_list, dtype=torch.float32)  # 确保数据类型正确

        # # # 将张量移动到 GPU
        # # merged_list = merged_list.to(device)

        # # 将 Python 列表转换为 NumPy 数组
        # data_np = np.array(merged_list)

        # dtrain = xgb.QuantileDMatrix(data_np, label=label_list)
        ...
        # 获得编码数据
        output_a, output_b, label_list, clusterid_list = get_encode_data(model_a, model_b, test_loader, device, pooling_strategy="cls")
        ...
        # # 将 Python 列表转换为 NumPy 数组
        # data_np_test = np.array(merged_list)

        # dtest = xgb.QuantileDMatrix(data_np_test)
    ...
    print(
        "Best Results for {}- Accuracy: {:.4f}, Precision: {:.4f}, Recall: {:.4f}, F1-Score: {:.4f}, AUROC: {:.4f}, AUPRC: {:.4f}".format(
            epitope_name, best_accuracy, best_precision, best_recall, best_f1, best_auroc, best_auprc))
