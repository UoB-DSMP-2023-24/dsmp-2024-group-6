...
def train_model(model, train_loader, val_loader, optimizer, loss_fn, device, epochs=10, early_stopping_patience=3):
    ...
    for epoch in range(epochs):
        ...
        # 对于多分类问题的AUPRC
        try:
            auprc = average_precision_score(all_labels, all_probabilities, average='macro')
        except ValueError as e:
            print(f"无法计算AUPRC: {e}")
            auprc = None
...
# 初始化模型、优化器和损失函数
model = tcr_bert_TwoPartBertClassifier_2.TwoPartBertClassifier(pretrained='/home/rnd/wmj/berlin/ttc/tcr-bert-mlm-only', n_output=30, freeze_encoder=False, separate_encoders=True, seq_pooling="mean")
...
train_df, remaining_data = train_test_split(df_alpha_beta, test_size=0.4, stratify=df_alpha_beta['antigen.epitope'], random_state=42)
validate_df, test_df = train_test_split(remaining_data, test_size=0.5, stratify=remaining_data['antigen.epitope'], random_state=42)
