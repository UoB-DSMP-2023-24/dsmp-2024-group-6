...
train_df, test_df = train_test_split(df_alpha_beta, test_size=0.2, stratify=df_alpha_beta['antigen.epitope'], random_state=42)
...
for a_row, b_row, v_b_gene, j_b_gene, v_a_gene, j_a_gene in zip(output_a, output_b, train_df['v_b_gene'], train_df['j_b_gene'], train_df['v_a_gene'], train_df['j_a_gene']):
    # 合并当前行的数据
...
merged_list = []
# 遍历 output_a 和 output_b 中的每一行
for a_row, b_row, v_b_gene, j_b_gene, v_a_gene, j_a_gene in zip(output_a, output_b, test_df['v_b_gene'], test_df['j_b_gene'], test_df['v_a_gene'], test_df['j_a_gene']):
    # 合并当前行的数据
    merged_row = a_row + b_row + [v_b_gene] + [j_b_gene] + [v_a_gene] + [j_a_gene]
    # 将合并后的行添加到结果列表中
    merged_list.append(merged_row)
