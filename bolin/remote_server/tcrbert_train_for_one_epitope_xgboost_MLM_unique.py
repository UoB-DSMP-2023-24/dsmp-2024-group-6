...
model_path =  "/home/rnd/wmj/berlin/ttc/tcr-bert"
output_path = "/home/rnd/wmj/berlin/ttc/log/training_log_beta.txt"
data_path = "/home/rnd/wmj/berlin/ttc/data_clean_large_neg.csv"
...
df_alpha_beta = df_alpha_beta[df_alpha_beta['antigen.epitope'] == 'KLGGALQAK']
...
for epitope in unique_epitopes:
    ...
    # 创建一个临时的唯一标识符列，不需要永久存储在DataFrame中
    df_filtered['temp_unique_id'] = df_filtered['cdr3_a_aa'].astype(str) + "__" + df_filtered['cdr3_b_aa'].astype(str)

    # 获取这个临时列的所有唯一值
    unique_ids = df_filtered['temp_unique_id'].unique()

    # 根据这些唯一标识符切分数据集
    train_ids, test_ids = train_test_split(unique_ids, test_size=0.2, random_state=42)

    # 通过这个临时标识符分配原始数据到训练集和测试集
    train_df = df_filtered[df_filtered['temp_unique_id'].isin(train_ids)]
    test_df = df_filtered[df_filtered['temp_unique_id'].isin(test_ids)]

    # 从训练集和测试集中删除临时列
    train_df.drop(columns=['temp_unique_id'], inplace=True)
    test_df.drop(columns=['temp_unique_id'], inplace=True)
    ...
    for epoch in range(0, epochs):
        ...
        # 训练模型
        clf.fit(output_b, label_list)

        # 获得编码数据
        output_a, output_b, label_list = get_encode_data(model_a, model_b, test_loader, device, pooling_strategy="cls")

        # merged_list = [a + b for a, b in zip(output_a, output_b)]

        # 预测测试集
        y_pred = clf.predict(output_b)

        y_score = clf.predict_proba(output_b)
